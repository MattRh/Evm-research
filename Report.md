...
В некоторых источниках их называют ERC(Ethereum Request for Comment) -
например `EIP 20`, легче ищется по названию `ERC 20` - но разработчики стандартов все равно больше придетшиваются названия EIP.
...
## Syntax
// todo: описание, что, где, как
Пример контракта, который отображает "дух" языка:
```solidity
pragma solidity ^0.4.24; // specify compiler version

import "../ownership/Heritable.sol"; // specify what external files we use here
 ...
 */
 // `SimpleSavingsWallet` inherits from `Heritable`
contract SimpleSavingsWallet is Heritable {
   ...
   */
  // fallback function. Can be called only by external contract
...
```
Польше примеров контрактов можно найти в `References -> Solidity -> Code Examples`
...
#### Getting/setting external property
// todo: solidity сам генерит геттер для public проперти, но не сеттер. Просто обратиться к проперти нельзя
...
### Events
// todo: функция, с которой надо разобраться, как обращаться
...
Пример:
```asm
// `mstore(0x40, 0x60)` at the very top reserves the first
// 64 bytes in memory for sha3 hashing.
...
// that is basically applying binary mask to first 
// 4 bytes of function hash to ensure it's format
0x0005 PUSH1 e0
0x0007 PUSH1 02
0x0009 EXP
0x000a PUSH1 00
...
В широком смысле ABI представляет из себя полное формальное определение всех функций в контракте,
дающее полную информацию об интерфейсе всего контракта.

## Inheritance
Т.к. у EVM нет сколь высоких абстракций, то Solidity добивается наследования простым копированием кода 
родительского класса в дочерний класс.

## Основные конструкции
Таблицу с байт-кодом основных конструкций Solidity можно найти в файле [lowlewel/mappings.md](https://github.com/MattRh/Evm-research/blob/master/lowlewel/mappings.md)
